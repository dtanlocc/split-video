...
# === CẤU HÌNH ===
API_KEYS = [
    "sk-43bbe53535ec43489c2a04a2695193b3" # Thay bằng API Key DeepSeek của bạn
]
# DeepSeek cung cấp model "deepseek-chat" cho các tác vụ thông thường và "deepseek-reasoner" cho lý luận. 
# Ở đây dùng deepseek-chat là tối ưu nhất.
...
def configure_model():
    ...
    with api_lock:
        ...
        # Khởi tạo OpenAI Client trỏ về DeepSeek
        client = OpenAI(
            api_key=api_key, 
...
def rotate_key(failed_key):
    ...
    with api_lock:
        ...
        if failed_key in active_keys:
            active_keys.remove(failed_key)
            print(f"⚠️ Loại bỏ API key lỗi/hết quota: {failed_key[:8]}... (Còn lại {len(active_keys)} keys hoạt động)")

        # Nếu danh sách rỗng -> Dừng toàn bộ chương trình
        if not active_keys:
            raise RuntimeError("❌ TOÀN BỘ API KEYS ĐÃ HẾT QUOTA HOẶC BỊ LỖI. CHƯƠNG TRÌNH DỪNG LẠI.")

        # Thiết lập lại client với key mới
        new_key = active_keys[0]
        client = OpenAI(
            api_key=new_key, 
            base_url="https://api.deepseek.com"
...
def build_highlight_prompt(subtitle_text):
    return (
        ...
        "Analyze the content and identify segments that are emotionally intense, action-packed, thought-provoking, or otherwise compelling highlights.\n"
        "Each selected segment must have complete meaning — do not cut off in the middle of a sentence or idea.\n"
        "Only include segments that are **at least 0 seconds** and **at most 500 seconds** long.\n"
        "Return the result as a JSON list in the following format:\n"
        '[{"start": "00:00:15,000", "end": "00:00:55,000"}, ...]\n\n'
        "Return **only plain JSON**, with no extra explanation or markdown formatting.\n\n"
        f"{subtitle_text}"
...
def generate_title(text_chunk):
    ...
    try:
        if response_text:
            ...
            return "(DeepSeek không trả về tiêu đề hợp lệ)"
    except Exception as e:
        return f"(Lỗi nội dung: {str(e)[:50]})"
...
def safe_generate_content(prompt, max_retries=8):
    """
    Gọi DeepSeek có retry, tự động chuyển API key và dừng khi hết tất cả key.
    """
    retries = 0
    while retries < max_retries:
        # Lấy key hiện hành để biết key nào gây ra lỗi (nếu có)
        with api_lock:
            if not active_keys:
                raise RuntimeError("❌ Tất cả API keys đều đã bị vô hiệu.")
            current_key = active_keys[0]
        ...
        try:
            response = client.chat.completions.create(
                ...
                temperature=0.7 # Có thể chỉnh nhiệt độ (sự sáng tạo) ở đây
            )
            # OpenAI / DeepSeek trả về text ở dạng này
            return response.choices[0].message.content

        except openai.RateLimitError:
            print(f"⚠️ Quá giới hạn Rate Limit / Quota với key {current_key[:8]}. Đang đổi key...")
            rotate_key(current_key) 
            time.sleep(2)
            continue
            
        except openai.AuthenticationError:
            print(f"⚠️ API Key không hợp lệ hoặc đã bị khóa: {current_key[:8]}. Đang đổi key...")
            rotate_key(current_key) 
            time.sleep(2)
            continue
            
        except openai.APIStatusError as e:
            # Bắt các mã lỗi như 402 (Payment Required), 429 (Too Many Requests)
            if e.status_code in [402, 403, 429]:
                print(f"⚠️ Lỗi Quota/Thanh toán (Mã {e.status_code}) với key {current_key[:8]}. Đang đổi key...")
                rotate_key(current_key)
                time.sleep(2)
                continue
            else:
                print(f"⚠️ Lỗi API Status ({retries+1}/{max_retries}): {e}")
                retries += 1
                time.sleep(3)
                
        except Exception as e:
            # Các lỗi khác (Mạng, server overload...)
            print(f"⚠️ Lỗi kết nối/Hệ thống ({retries+1}/{max_retries}): {str(e)[:80]}...")
            retries += 1
            time.sleep(3)
...
def process_video(srt_path: Path):
    try:
        ...
        print(f"\n🎬 Đang xử lý video: {video_name}")
        response_text = safe_generate_content(build_highlight_prompt(subtitle_text))
        segments = extract_json_from_text(response_text)
